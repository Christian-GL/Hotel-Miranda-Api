...
var RoomValidator = /** @class */ (function () {
    ...
    RoomValidator.prototype.validateRoom = function (room, allRooms, allBookings) {
        ...
        // )
        this.validateNumber(room.number, false, allRooms).map(function (error) { return allErrorMessages.push(error); });
        ...
        this.validateBookingList(room.booking_list, allBookings).map(function (error) { return allErrorMessages.push(error); });
        ...
        bookingList.map(function (booking) {
            ...
            bookingRequiredProperties.map(function (property) {
                if (!(property in booking)) {
                    errorMessages.push("Property [".concat(property, "] is required in Booking of booking_list"));
                    return errorMessages;
                }
            });
            // VALIDAR FOTO
            ...
            bookingValidator.validateCheckInCheckOut(booking.check_in_date, booking.check_out_date).map(function (error) { return errorMessages.push(error); });
            bookingValidator.validateDateIsOccupied(booking.check_in_date, booking.check_out_date, allBookings).map(function (error) { return errorMessages.push(error); });
            bookingValidator.validateBookingStatus(booking.status).map(function (error) { return errorMessages.push(error); });
